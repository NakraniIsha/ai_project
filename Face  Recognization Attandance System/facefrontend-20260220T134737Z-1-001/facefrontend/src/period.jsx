...
import { FaHome, FaFileAlt, FaUser, FaDownload, FaClock, FaCog } from 'react-icons/fa';
...
const Period = () => {
  ...
  useEffect(() => {
    ...
    const fetchPeriods = async () => {
      try {
        const response = await fetch("http://localhost:5001/api/periods");
        const data = await response.json();
        setPeriods(data);
      } catch (error) {
          ...
          <div className="bg-white rounded-[1.1rem] shadow-md p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 mt-5">
              {periods.map((period) => {
                ...
                return (
                  <div key={period._id} className="p-5 rounded-2xl bg-gray-200 flex items-center justify-between shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h2 className="font-semibold text-xl text-gray-800">{period.name}</h2>
                        <h1 className="text-sm text-gray-500">{period.time}</h1>
                      </div>
                    </div>
